...
- **Dropped rapid input** (`catastrophe.h`): debounce now applies per button to fresh presses only. Releases always pass through and no longer consume the debounce window for the next legitimate press; generated directional repeats keep using their own rate control.
