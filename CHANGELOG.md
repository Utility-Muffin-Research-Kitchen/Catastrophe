...
- **Animated battery indicator** (`catastrophe.h`): the status-bar battery now animates a rising green fill while charging (~3s sweep) and flashes red when critically low (≤10%, unplugged); normal level uses the themed fill. Self-driving via `cat_request_frame_in` only while charging/low (no extra redraws otherwise), so it animates in any app using the status bar. Also fixed the normal fill to be **left-anchored** (drains/fills left-to-right like a real battery) instead of right-anchored.
- **Analog-stick click button** (`catastrophe.h`): new `CAT_BTN_STICK` in the button enum. On MLP1 the stick click (`BTN_THUMBL`) was previously mapped to `CAT_BTN_NONE` (discarded); it now surfaces as `CAT_BTN_STICK` so apps can bind it. Added before `CAT_BTN_COUNT` (no renumbering of existing buttons).
- **Per-corner rounded rect** (`catastrophe.h`): `cat_draw_rounded_rect_ex(x, y, w, h, r, corners, color)` rounds only the corners in a `CAT_CORNER_TL/TR/BL/BR` bitmask (others square); `cat_draw_rounded_rect` is now a passthrough with `CAT_CORNER_ALL`. New `ap_theme.pill_corner_mask` lets `cat_draw_pill` draw asymmetric pills (0/ALL = the default full pill) — used for the Jawaka "Leaf" list style (bottom-left + top-right rounded).
- **Triangle primitive** (`catastrophe.h`): public `cat_draw_triangle(x, y, w, h, cat_dir dir, color)` fills a solid triangle inside the given box pointing `dir` (`CAT_DIR_LEFT/RIGHT/UP/DOWN`) — a reusable affordance for value cyclers, scroll hints, etc. Built on the existing internal `cat__fill_triangle` (`SDL_RenderGeometry`); the tab-bar overflow arrows now draw through this same helper, so there is a single triangle implementation.
- **Textured quad primitive** (`catastrophe.h`): public `cat_draw_textured_quad(tex, corners, uv, col)` draws one quad (two triangles) via `SDL_RenderGeometry` — `tex` NULL fills a flat colour, `uv` NULL uses the full 0..1 texture, `col` NULL is opaque white; corners are TL,TR,BR,BL. Consolidates two ad-hoc `SDL_RenderGeometry` quad helpers that had lived in the Jawaka launcher (a solid parallelogram filler and the Cover Flow card quad), so there is a single quad implementation. For perspective or tilted cards, subdivide into vertical strips before calling to avoid the affine-mapping shear along the split diagonal.
...
- **List cursor wraps around** (`catastrophe_widgets.h`): `cat_list_state_move()` now wraps top<->bottom instead of clamping at the ends, matching tab navigation. Pressing Up on the first row jumps to the last and vice-versa. Modulo arithmetic keeps it correct for any delta magnitude. `cat_list_state_page()` / `cat_list_state_jump()` still clamp.
- **Tighter marquee loop gap** (`catastrophe.h`): the looping `cat_draw_text_marquee` gap before the wrap-around copy is now `visible_w / 9` (min `CAT_S(10)`), down from `visible_w / 3` (min 28), so scrolling text repeats sooner.
...
- **Dropped button presses after a slow frame** (`catastrophe.h`): `cat_poll_input()` debounced on the time an event was *popped* rather than the time it arrived, so a frame long enough to queue several events (a cover-art decode, a DB query) drained them all with one identical `SDL_GetTicks()` timestamp and discarded everything after the first inside the 20 ms window. Events are now stamped on arrival (`cat__input_queue_ms`, parallel to the ring — no change to the public `cat_input_event`), so a slow frame costs latency instead of input. Button *releases* also no longer gate or arm the debounce window: callers discard them, and an accepted release used to hold the budget just long enough to eat the press that followed it — the "d-pad tap that does nothing" seen roughly one navigation in twenty.
- **Options-list timed refresh** (`catastrophe_widgets.h`, `examples/demo/main.c`): `refresh_interval_ms` now returns promptly after its scheduled wake instead of falling through to a final idle `cat_present()` that could sleep until the next wall-clock minute. The smoke check now exercises two refreshes after the initial pill animation while preserving focus and scroll.
